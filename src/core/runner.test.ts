...
test("requestRunSpecs suprime retrospectivas sistemicas por feature flag e mantem o fluxo funcional", async () => {
  const fixture = await setupRunSpecsFixture([ticketA.name], {
  ...
  try {
    ...
    codex.specTicketValidationAutoCorrectHandler = createSpecTicketValidationAutoCorrectHandler(
      fixture.projectRoot,
      "Cobertura corrigida para liberar a auditoria funcional.",
    );
    codex.specTicketValidationTurns = [
      createSpecTicketValidationPassResult({
        verdict: "NO_GO",
        confidence: "high",
        summary: "Primeiro passe encontrou gap auto-corrigivel antes da retrospectiva.",
        gaps: [
          {
            gapType: "coverage-gap",
            summary: "RF-01 ainda sem ticket dedicado.",
    ...
    codex.stageOutputs["spec-ticket-derivation-retrospective"] = createWorkflowGapAnalysisOutput({
      summary: "Esta retrospectiva nao deveria rodar com a feature flag desligada.",
      publicationEligibility: true,
    });
    codex.stageOutputs["spec-workflow-retrospective"] = createWorkflowGapAnalysisOutput({
      summary: "Esta retrospectiva pos-auditoria nao deveria rodar com a feature flag desligada.",
      publicationEligibility: true,
    });
    ...
    assert.equal(runSpecsSummary.specTicketValidation?.verdict, "GO");
    assert.equal(runSpecsSummary.specTicketDerivationRetrospective, undefined);
    assert.equal(runSpecsSummary.workflowGapAnalysis, undefined);
    assert.equal(runSpecsSummary.workflowImprovementTicket, undefined);
    assert.deepEqual(runSpecsSummary.triageTiming.completedStages, [
      "spec-triage",
      "spec-ticket-validation",
      "spec-close-and-version",
    ]);
    ...
    assert.equal(
      runSpecsSummary.triageTiming.durationsByStageMs["spec-ticket-derivation-retrospective"],
      undefined,
    );
    assert.equal(
      runSpecsSummary.timing.durationsByStageMs["spec-workflow-retrospective"],
      undefined,
    );

    const suppressedRetrospectiveTraces = records.filter(
      (entry) =>
        entry.request.stage === "spec-ticket-derivation-retrospective" ||
        entry.request.stage === "spec-workflow-retrospective",
    );
    assert.deepEqual(
      suppressedRetrospectiveTraces.map((entry) => ({
        stage: entry.request.stage,
        promptTemplatePath: entry.request.promptTemplatePath,
        suppressedByFeatureFlag: entry.request.decision.metadata?.suppressedByFeatureFlag,
      })),
      [
        {
    ...
    assert.equal(
      logger.infos.some(
        (entry) => entry.message === "Retrospectiva pre-run-all suprimida por feature flag",
      ),
      true,
    );
    assert.equal(
      logger.infos.some(
        (entry) => entry.message === "Retrospectiva pos-spec-audit suprimida por feature flag",
      ),
      true,
    );
    ...
    assert.doesNotMatch(
      specContent,
      /Esta retrospectiva nao deveria rodar com a feature flag desligada\./u,
    );
  } finally {
    await cleanupTempProjectRoot(fixture.projectRoot);
  }
...
test("requestTargetPrepare inicia lifecycle assincrono, expoe estado target e permite trocar o projeto ativo", async () => {
  const logger = new SpyLogger();
  const roundDependencies = createRoundDependencies({
    activeProject: activeProjectA,
    queue: defaultQueue,
  ...
  assert.deepEqual(syncResult, { status: "updated" });
  assert.equal(runner.getState().activeProject?.name, "gamma-project");
...
test("requestRunAll pode coexistir com /target_prepare em outro projeto", async () => {
  const logger = new SpyLogger();
  const roundDependencies = createRoundDependencies({
    activeProject: activeProjectA,
    queue: defaultQueue,
    codexClient: new StubCodexClient(undefined, true, false, 20),
    gitVersioning: new StubGitVersioning(),
  });
  const release = createDeferred<void>();
  const runner = createRunner(logger, roundDependencies, {
      ...
      targetPrepareExecutor: {
        execute: async (projectName, hooks) => {
          ...
          await hooks?.onMilestone?.({
            ...
            message: `Preflight concluido para ${projectName}.`,
            versionBoundaryState: "before-versioning",
            recordedAtUtc: "2026-03-24T23:30:00.000Z",
          });
          await release.promise;
          return {
            ...
            summary: {
              ...
              changedPaths: [],
              nextAction: "Reexecute quando quiser retomar o preparo.",
            },
          };
        },
      },
  ...
  const targetResult = await runner.requestTargetPrepare("beta-target");
  const runAllResult = await runner.requestRunAll();
  ...
  assert.deepEqual(runAllResult, { status: "started" });
  assert.equal(runner.getState().activeSlots.length, 2);
  assert.match(
    runner.getState().activeSlots.map((slot) => `${slot.project.name}:${slot.kind}`).join(","),
    /beta-target:target-prepare/u,
  );
  assert.match(
    runner.getState().activeSlots.map((slot) => `${slot.project.name}:${slot.kind}`).join(","),
    /alpha-project:run-all/u,
  );
...
test("requestRunAll bloqueia o mesmo projeto quando /target_prepare ocupa o slot", async () => {
  const logger = new SpyLogger();
  const roundDependencies = createRoundDependencies({
    activeProject: activeProjectA,
    queue: defaultQueue,
      ...
      targetPrepareExecutor: {
        execute: async (projectName, hooks) => {
          ...
          await hooks?.onMilestone?.({
            ...
            message: `Preflight concluido para ${projectName}.`,
            versionBoundaryState: "before-versioning",
            recordedAtUtc: "2026-03-24T23:31:00.000Z",
          });
          await release.promise;
          return {
            status: "cancelled" as const,
  ...
  await runner.requestTargetPrepare("alpha-project");
  const runAllResult = await runner.requestRunAll();
  const planSpecResult = await runner.startPlanSpecSession("42");

  assert.equal(runAllResult.status, "blocked");
  assert.equal(runAllResult.reason, "project-slot-busy");
  assert.match(runAllResult.message, /slot do projeto alpha-project ja ocupado por \/target_prepare/u);
  assert.equal(planSpecResult.status, "blocked");
  if (planSpecResult.status === "blocked") {
    assert.equal(planSpecResult.reason, "project-slot-busy");
    assert.match(planSpecResult.message, /slot do projeto alpha-project ja ocupado por \/target_prepare/u);
  }
...
test("requestTargetPrepare permite fluxos target em projetos diferentes e exige escopo para cancelar", async () => {
  const logger = new SpyLogger();
  const roundDependencies = createRoundDependencies({
    activeProject: activeProjectA,
    queue: defaultQueue,
      ...
      targetPrepareExecutor: {
        execute: async (projectName, hooks) => {
          ...
          await hooks?.onMilestone?.({
            ...
            message: `Preflight concluido para ${projectName}.`,
            versionBoundaryState: "before-versioning",
            recordedAtUtc: "2026-03-24T23:32:00.000Z",
          });
          await getRelease(projectName).promise;
          if (hooks?.isCancellationRequested?.()) {
            return {
              ...
              summary: {
                ...
                cancelledAtMilestone: "preflight" as const,
                changedPaths: [],
                nextAction: "Reexecute o preparo quando quiser continuar.",
              },
            };
          }
          ...
          return {
            ...
            summary: {
              ...
              eligibleForProjects: true,
              compatibleWithWorkflowComplete: true,
              nextAction: "Selecionar o projeto pelo menu /projects.",
              manifestPath: "docs/workflows/target-prepare-manifest.json",
              reportPath: "docs/workflows/target-prepare-report.md",
              changedPaths: [
                "docs/workflows/target-prepare-manifest.json",
              ...
              versioning: {
                status: "committed-and-pushed" as const,
                commitHash: `prepare-${projectName}`,
                upstream: "origin/main",
                commitPushId: `prepare-${projectName}@origin/main`,
              },
            },
          };
        },
  ...
  const firstStart = await runner.requestTargetPrepare("alpha-project");
  const secondStart = await runner.requestTargetPrepare("beta-project");
  ...
  assert.deepEqual(secondStart, {
    status: "started",
    message: "Execucao /target_prepare iniciada para beta-project.",
  });
  assert.equal(
    runner.getState().activeSlots.filter((slot) => slot.kind === "target-prepare").length,
    2,
  );
  ...
  const ambiguousCancel = await runner.cancelTargetPrepare();
  assert.equal(ambiguousCancel.status, "ambiguous");
  assert.match(ambiguousCancel.message, /Existem 2 execucoes \/target_prepare ativas/u);

  runner.syncActiveProject(activeProjectA);
  const scopedCancel = await runner.cancelTargetPrepare();
            ...
            summary: {
              ...
              tracePayload: {
                ...
                semantic_review: {
                  ...
                  symptom: null,
                  symptom_selection_source: null,
                  selected_candidate_id: null,
                  symptom_candidate_count: 0,
...
test("requestTargetInvestigateCase preserva flow e comando v2 no lifecycle ativo e no resumo final", async () => {
  const logger = new SpyLogger();
  const roundDependencies = createRoundDependencies({
    activeProject: activeProjectA,
    queue: defaultQueue,
  ...
  const runner = createRunner(logger, roundDependencies, {
    runnerOptions: {
      ...
      targetFlowEventHandlers: {
        onMilestone: async (event) => {
          milestoneEvents.push(`${event.flow}:${event.command}:${event.milestone}`);
        },
      },
      targetInvestigateCaseExecutor: {
        execute: async (request, hooks) => {
          assert.notEqual(typeof request.input, "string");
          if (typeof request.input === "string" || !("canonicalCommand" in request.input)) {
            throw new Error("unexpected string input");
          }
          assert.match(request.input.canonicalCommand, /^\/target_investigate_case_v2 /u);
          const targetProject = {
            name: "beta-target",
            path: "/home/mapita/projetos/beta-target",
          };
          await hooks?.onMilestone?.({
            ...
            message: "Preflight concluido para beta-target.",
            versionBoundaryState: "before-versioning",
            recordedAtUtc: "2026-04-03T19:10:00.000Z",
          });
          await release.promise;
          return {
            status: "cancelled" as const,
            summary: {
              targetProject,
              roundId: "2026-04-03T19-10-00Z",
              roundDirectory: "output/case-investigation/2026-04-03T19-10-00Z",
              artifactPaths: [],
              cancelledAtMilestone: "preflight" as const,
              nextAction: "Reexecute quando quiser continuar a investigacao v2.",
              versionBoundaryState: "before-versioning" as const,
            },
          };
        },
  ...
  const started = await runner.requestTargetInvestigateCase(
    "/target_investigate_case_v2 beta-target case-001 --workflow extract_address --request-id req-001",
  );

  assert.deepEqual(started, {
    status: "started",
    message: "Execucao /target_investigate_case_v2 iniciada para beta-target.",
  });
  assert.equal(runner.getState().targetFlow?.flow, "target-investigate-case-v2");
  assert.equal(runner.getState().targetFlow?.command, "/target_investigate_case_v2");
  assert.equal(runner.getState().activeSlots[0]?.kind, "target-investigate-case-v2");
  assert.equal(runner.getState().phase, "target-investigate-case-v2-preflight");
  assert.deepEqual(milestoneEvents, [
  ...
  assert.equal(flowSummaries[0]?.flow, "target-investigate-case-v2");
  assert.equal(flowSummaries[0]?.command, "/target_investigate_case_v2");
  assert.equal(flowSummaries[0]?.outcome, "cancelled");
});
          ...
          return {
            ...
            summary: {
              ...
              artifactPaths: {
                ...
                diagnosisJsonPath: "investigations/2026-04-03T20-00-00Z/diagnosis.json",
                diagnosisMdPath: "investigations/2026-04-03T20-00-00Z/diagnosis.md",
              ...
              realizedArtifactPaths: [
                ...
                "investigations/2026-04-03T20-00-00Z/diagnosis.json",
                "investigations/2026-04-03T20-00-00Z/diagnosis.md",
              ...
              finalSummary: {
                ...
                diagnosis: {
                  ...
                  summary: "O diagnostico permaneceu inconclusivo neste cenario de teste.",
                  why: "A rodada de teste nao reuniu sinal suficiente para concluir OK ou NOT_OK.",
                  expected_behavior: "Fechar o caso apenas quando houver evidencia suficiente.",
                  observed_behavior: "A fixture preserva o caso como inconclusivo.",
                  confidence: "medium",
                  behavior_to_change: "Coletar evidencia adicional antes de concluir o caso.",
                  probable_fix_surface: ["coleta-adicional"],
                  next_action: "Revise a rodada.",
                  bundle_artifact: "investigations/2026-04-03T20-00-00Z/evidence-bundle.json",
                  diagnosis_md_path: "investigations/2026-04-03T20-00-00Z/diagnosis.md",
                  diagnosis_json_path: "investigations/2026-04-03T20-00-00Z/diagnosis.json",
                },
              ...
              tracePayload: {
                ...
                diagnosis: {
                  ...
                  lineage: [
                    {
                      artifact: "assessment.json",
                      path: "investigations/2026-04-03T20-00-00Z/assessment.json",
                    },
                  ],
                },
                verdicts: {
          ...
          return {
            ...
            summary: {
              ...
              artifactPaths: {
                ...
                diagnosisJsonPath: "investigations/2026-04-03T21-00-00Z/diagnosis.json",
                diagnosisMdPath: "investigations/2026-04-03T21-00-00Z/diagnosis.md",
              ...
              realizedArtifactPaths: [
                ...
                "investigations/2026-04-03T21-00-00Z/diagnosis.json",
                "investigations/2026-04-03T21-00-00Z/diagnosis.md",
              ...
              finalSummary: {
                ...
                diagnosis: {
                  ...
                  why: "A evidencia confirma falha do guardrail local com superficie de correcao definida.",
                  expected_behavior: "Preservar o guardrail local do billing-core.",
                  observed_behavior: "O guardrail local falhou no caso publicado.",
                  confidence: "high",
                  behavior_to_change: "Corrigir o guardrail local reutilizavel.",
                  probable_fix_surface: ["src/workflows/billing-core.ts"],
                  next_action: "Revisar o ticket publicado.",
                  bundle_artifact: "investigations/2026-04-03T21-00-00Z/evidence-bundle.json",
                  diagnosis_md_path: "investigations/2026-04-03T21-00-00Z/diagnosis.md",
                  diagnosis_json_path: "investigations/2026-04-03T21-00-00Z/diagnosis.json",
                },
                houve_gap_real: "yes",
              ...
              tracePayload: {
                ...
                diagnosis: {
                  ...
                  lineage: [
                    {
                      artifact: "assessment.json",
                      path: "investigations/2026-04-03T21-00-00Z/assessment.json",
                    },
                  ],
                },
                verdicts: {
