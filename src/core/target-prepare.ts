...
export class ControlledTargetPrepareExecutor implements TargetPrepareExecutor {
  ...
  private renderReport(params: {
    ...
    return [
      "# Target Prepare Report",
      "",
      "## Summary",
      `- Generated at (UTC): ${generatedAtUtc}`,
      `- Runner reference: ${params.runnerReference}`,
      `- Target project: ${params.targetProject.name}`,
      `- Target path: ${params.targetProject.path}`,
      `- Eligible for /projects: yes`,
      `- Compatible with workflow complete: yes`,
      `- Recommended next action: Selecionar o projeto por /select_project ${params.targetProject.name} ou pelo menu /projects.`,
      "",
      "## Git snapshot",
      ...
      "",
      "## Notes",
      "- Manifesto tecnico e relatorio humano foram gerados pelo runner apos pos-check deterministico.",
      "- Commit/push sao permitidos apenas depois deste relatorio existir e dos validadores estarem verdes.",
      "",
    ].join("\n");
  }
}
