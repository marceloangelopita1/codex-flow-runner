...
interface GitCliTargetDeriveGuardDependencies {
  runGit: (args: string[]) => Promise<GitCommandResult>;
}

export interface TargetDeriveGitGuard {
...
export class TargetDeriveWorkingTreeDirtyError extends Error {
  constructor(public readonly statusOutput: string) {
    super(
      [
        "O target_derive_gaps exige working tree limpo antes de iniciar.",
        `Status atual: ${statusOutput || "(nao informado)"}.`,
      ].join(" "),
    );
    this.name = "TargetDeriveWorkingTreeDirtyError";
  }
}

export class GitCliTargetDeriveGuard implements TargetDeriveGitGuard {
  private readonly dependencies: GitCliTargetDeriveGuardDependencies;

  constructor(
    private readonly repoPath: string,
    dependencies: Partial<GitCliTargetDeriveGuardDependencies> = {},
  ) {
    this.dependencies = {
      runGit: async (args) =>
  ...
  async assertCleanWorkingTree(): Promise<void> {
    ...
    if (statusOutput.length > 0) {
      throw new TargetDeriveWorkingTreeDirtyError(statusOutput);
    }
  }
