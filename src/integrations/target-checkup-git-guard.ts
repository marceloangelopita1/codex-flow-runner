...
interface GitCliTargetCheckupGuardDependencies {
  runGit: (args: string[]) => Promise<GitCommandResult>;
}

export interface TargetCheckupGitGuard {
  assertCleanWorkingTree(): Promise<void>;
  listChangedPaths(): Promise<string[]>;
  getCurrentBranch(): Promise<string | null>;
  getHeadSha(): Promise<string | null>;
  getHeadParentSha(): Promise<string | null>;
  getLastCommitTouchingPath(relativePath: string): Promise<string | null>;
}

export class TargetCheckupWorkingTreeDirtyError extends Error {
  constructor(public readonly statusOutput: string) {
    super(
      [
        "O target_checkup exige working tree limpo antes de iniciar.",
        `Status atual: ${statusOutput || "(nao informado)"}.`,
      ].join(" "),
    );
    this.name = "TargetCheckupWorkingTreeDirtyError";
  }
}

export class GitCliTargetCheckupGuard implements TargetCheckupGitGuard {
  private readonly dependencies: GitCliTargetCheckupGuardDependencies;

  constructor(
    private readonly repoPath: string,
    dependencies: Partial<GitCliTargetCheckupGuardDependencies> = {},
  ) {
    this.dependencies = {
      runGit: async (args) =>
  ...
  async assertCleanWorkingTree(): Promise<void> {
    ...
    if (statusOutput.length > 0) {
      throw new TargetCheckupWorkingTreeDirtyError(statusOutput);
    }
  }
  ...
  async getLastCommitTouchingPath(relativePath: string): Promise<string | null> {
    try {
      const value = (
        await this.runGit(["log", "-n", "1", "--format=%H", "--", relativePath])
      ).stdout.trim();
      return value || null;
    } catch {
      return null;
